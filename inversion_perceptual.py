...
class Inversor():
    ...
    def log_cosh_loss(self, x_pred, x_p):
        def _log_cosh(x):
    ...
    def inverse(self, imgname, total_step, lr, mse_w, lpip_w, truncation_psi, noise_mode, space='z'):
        ...
        img_str = imgname.split('/')[-1].split('.')[0]
        ...
        perceptual_net = VGG16_for_Perceptual().cuda().eval()
        ...
        for step_idx in tqdm(range(total_step), total=total_step):
            ...
            # print('\n\n\n\n\n')
            # losses
            mseloss1 = self.MSE_C(inv_image, gt_image) #*mse_w
            mseloss = self.log_cosh_loss(inv_image, gt_image)
            ...
            #lpipsloss1 = caluclate_loss(self.resize_image(inv_image), self.resize_image(gt_image), perceptual_net)
            # ssimloss = self.SSIM(inv_image, gt_image)
            loss = mseloss1 + mseloss+ lpipsloss * 15 #+ lpipsloss1 
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            scheduler.step()
...
def caluclate_loss(synth_img, img_p, perceptual_net):
...
@click.option('--lr', type=float, default=4e-2, help='learning rate')
...
@click.option('--space', type=click.Choice(['z', 'wp']), default='wp') 
